// db/schema.ts
// Drizzle ORM schema — Postgres (Neon / Supabase)
//
...
// products.lastCostPaise are just defaults the UI pre-fills — every sale line
// and purchase line stores its OWN price, entered fresh each time.
...
export const categories = pgTable("categories", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 80 }).notNull().unique(), // Cigarette, Nepnawi, Vur, Food
  active: boolean("active").notNull().default(true),
});
...
export const products = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    sku: varchar("sku", { length: 40 }).unique(), // optional, can be null for now
    name: varchar("name", { length: 120 }).notNull(), // e.g. "V (Mint)", "Momo"
    categoryId: integer("category_id").notNull().references(() => categories.id),
    type: productType("type").notNull().default("resale"),
    unit: varchar("unit", { length: 20 }).notNull().default("pcs"), // pcs, plate, glass...

    // Convenience defaults only — never the authoritative price.
    // The UI pre-fills a sale/purchase form with these; every transaction
...
export const purchaseLines = pgTable("purchase_lines", {
  ...
  unitPricePaise: bigint("unit_price_paise", { mode: "bigint" }).notNull(), // entered fresh, every time
...
export const productionBatches = pgTable("production_batches", {
  id: serial("id").primaryKey(),
  productId: integer("product_id").notNull().references(() => products.id),
  batchDate: date("batch_date").notNull(), // e.g. 2026-09-24
  qtyProduced: integer("qty_produced").notNull(),
  totalCostPaise: bigint("total_cost_paise", { mode: "bigint" }).notNull(), // ingredients + labour, entered by hand
  notes: text("notes"), // e.g. "momo, chicken + veg mix"
  status: documentStatus("status").notNull().default("draft"),
  createdBy: integer("created_by").notNull().references(() => users.id),
  postedAt: timestamp("posted_at", { withTimezone: true }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
...
export const sales = pgTable("sales", {
  ...
  paymentStatus: varchar("payment_status", { length: 12 }).notNull().default("paid"), // paid | partial | unpaid
  status: documentStatus("status").notNull().default("draft"),
...
export const saleLines = pgTable("sale_lines", {
  ...
  unitPricePaise: bigint("unit_price_paise", { mode: "bigint" }).notNull(), // entered fresh at sale time
  discountPaise: bigint("discount_paise", { mode: "bigint" }).notNull().default(0n),
...
export const stockMovements = pgTable(
  "stock_movements",
  {
    ...
    sourceType: varchar("source_type", { length: 30 }).notNull(), // "purchase" | "sale" | "production" | ...
...
// ---------------------------------------------------------------------------
// Stock adjustments (damage, loss, count correction)
// ---------------------------------------------------------------------------
...
export const stockAdjustments = pgTable("stock_adjustments", {
  ...
  reasonType: varchar("reason_type", { length: 20 }).notNull(), // damage | loss | found | correction | sample
...
export const expenseCategories = pgTable("expense_categories", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 60 }).notNull().unique(), // Rent, Transport, Utilities...
  active: boolean("active").notNull().default(true),
});
...
// ---------------------------------------------------------------------------
// Payments (settle sales/purchases; separate from the sale/purchase itself)
// ---------------------------------------------------------------------------
...
export const payments = pgTable("payments", {
  ...
  sourceType: varchar("source_type", { length: 20 }).notNull(), // "sale" | "purchase"
  sourceId: integer("source_id").notNull(),
});
...
export const auditEvents = pgTable(
  "audit_events",
  {
    ...
    action: varchar("action", { length: 40 }).notNull(), // "post", "reverse", "approve", "create", ...
